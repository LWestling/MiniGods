...
public class MaterialManager {
    ...
    // bad design TODO FIX
    public void render(int renderPass, int cameraLocation, int lightLocation, int cameraPosLocation, AnimatedModelMaterial... animatedMaterials) {
        RenderPass pass = renderPasses.get(renderPass);
        ...
        for (AnimatedModelMaterial animatedMaterial : animatedMaterials) {
            render(animatedMaterial);
        }
    }
    ...
    public void render(AnimatedModelMaterial animatedModelMaterial) {
        Material material = animatedModelMaterial.getMaterial();

        for (AnimatedModelMaterial.Uniform uniform : animatedModelMaterial.getUniforms()) {
            GLES31.glUniformMatrix4fv(uniform.location, uniform.size, false, uniform.buffer);
        }

        Indices indices = material.getIndices();
        ImageTexture imageTexture = material.getImageTexture();

        for (int vao : animatedModelMaterial.getVao()) { // always 1
            GLES31.glBindVertexArray(vao);
            ...
            if (indices != null) {
                ...
                GLES31.glDrawArrays(GLES31.GL_TRIANGLES, 0, animatedModelMaterial.getVertexCount());
            }
